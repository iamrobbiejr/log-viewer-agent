...
Cloudflare Tunnels securely expose the PDS Log Agent running on a local machine (e.g., port 8000) to the internet without opening any router ports or configuring firewall rules. It provides a static, permanent HTTPS URL for each machine.
...
2. Copy the command provided in the "Run the following command" box. It usually looks like this:
   ...
   ```
3. On the production machine where the PDS Log Agent is installed:
...
## Step 4: Add the Machine to the RevMax Dashboard

Your machine is now live on the internet! 

1. Open your RevMax Log Viewer Dashboard.
...
5. Open the `config.json` on the production machine and update `agent_secret` to match the secret you just generated. Restart the PDS Log Agent service.

**Done!** Your Vercel backend can now securely query this specific machine forever, regardless of its local IP address.
